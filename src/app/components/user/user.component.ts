...
export class UserComponent implements OnInit {
  ...
  updateOptionsByKey(formCreation: any, key: string, newOptions: any): any {
    const field = formCreation.find((item: any) => item.key === key);
    if (field) {
  ...
  constructor(private userService: UserService, private toasterService: ToasterService, private breakpointObserver: BreakpointObserver, private storageService: StorageService) {
  ...
  ngOnInit(): void {
    this.getUsers();
    if (this.storageService.extractToken().includes("ADMIN")) {
      this.formCreation = this.updateOptionsByKey(this.formCreation, "role", ["ADMIN", "USER"]);
      this.newformCreation = this.updateOptionsByKey(this.newformCreation, "role", ["ADMIN", "USER"]);
    }
